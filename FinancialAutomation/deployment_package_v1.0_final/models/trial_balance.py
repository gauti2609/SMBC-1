...
class TrialBalance:
        ...
        if import_batch_id:
            cursor.execute('''
                SELECT tb_id, company_id, ledger_name,
                       ...
                       is_mapped, import_batch_id
                FROM trial_balance
                WHERE company_id = ? AND is_mapped = 0 AND import_batch_id = ?
                ORDER BY ledger_name
            ''', (company_id, import_batch_id))
        else:
            cursor.execute('''
        ...
        try:
            cursor.execute('''
                ...
                SET major_head_id = ?, minor_head_id = ?, grouping_id = ?,
                    type_bs_pl = ?, is_mapped = 1, updated_at = ?
                WHERE tb_id = ?
    ...
    def update_values(tb_id, opening_balance_cy=None, debit_cy=None, credit_cy=None, 
        ...
        try:
            ...
            if updates:
                ...
                query = f"UPDATE trial_balance SET {', '.join(updates)} WHERE tb_id = ?"
    ...
    def delete(tb_id):
        ...
        cursor.execute('DELETE FROM trial_balance WHERE tb_id = ?', (tb_id,))
    ...
    def delete_by_company(company_id, import_batch_id=None):
        ...
        if import_batch_id:
            cursor.execute('DELETE FROM trial_balance WHERE company_id = ? AND import_batch_id = ?',
                          (company_id, import_batch_id))
        else:
