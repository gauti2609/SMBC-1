...
class LoginWindow(QWidget):
    ...
    def handle_login(self):
        ...
        try:
            ...
            if success:
                # Don't show success message - just open main window directly
    ...
    def open_main_window(self, user):
        ...
        try:
            ...
            # Make sure main window is visible and activated
            self.main_window.show()
            self.main_window.raise_()
            self.main_window.activateWindow()
            print("Main window shown and activated")
            
            # Only close login window if main window initialized successfully
            ...
            QMessageBox.critical(
                self,
                "Startup Error",
                f"Failed to open main application window:\n\n{str(e)}\n\nPlease check:\n" +
                "1. Database connection is configured correctly (.env file)\n" +
                "2. Database server is running (PostgreSQL or SQLite)\n" +
                "3. Database tables are initialized\n\n" +
                "The login window will remain open."
            )
            # Keep login window open so user can try again or exit properly
