...
class MainWindow(QMainWindow):
    ...
    def __init__(self, user):
        super().__init__()
        print(f"MainWindow.__init__ started for user: {user.username}")
        self.user = user
        ...
        try:
            ...
            import traceback
            traceback.print_exc()
            # Re-raise the exception so login window can handle it
    ...
    def new_company(self):
        ...
        try:
            if hasattr(self, 'company_info_tab') and self.company_info_tab is not None:
                self.tab_widget.setCurrentWidget(self.company_info_tab)
                self.company_info_tab.clear_form()
                self.update_status_bar("Ready to create new company - Fill the form and click 'Save Company Info'")
            else:
                QMessageBox.warning(self, "Error", "Company Info tab not initialized")
        except Exception as e:
            print(f"Error in new_company: {e}")
            import traceback
            traceback.print_exc()
            QMessageBox.critical(self, "Error", f"Failed to open new company form:\n{str(e)}")
