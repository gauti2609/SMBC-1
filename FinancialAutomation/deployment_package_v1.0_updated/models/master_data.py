...
class MajorHead:
    ...
    def get_by_id(major_head_id):
        ...
        cursor.execute('''
            ...
            FROM major_heads
            WHERE major_head_id = ? AND is_active = 1
        ''', (major_head_id,))
    ...
    def get_by_name(name):
        ...
        cursor.execute('''
            ...
            FROM major_heads
            WHERE major_head_name = ? AND is_active = 1
        ''', (name,))
    ...
    def create(company_id, major_head_name, category, opening_balance_cy=0.0, 
        ...
        try:
            ...
            cursor.execute(
                'SELECT MAX(display_order) FROM major_heads WHERE company_id = ?',
                (company_id,)
            )
            ...
            major_head_id = cursor.lastrowid
            conn.commit()
            conn.close()
...
class MinorHead:
    ...
    def get_all(company_id=None, major_head_id=None):
        ...
        if company_id and major_head_id:
            cursor.execute('''
                ...
                FROM minor_heads
                WHERE company_id = ? AND major_head_id = ? AND is_active = 1
    ...
    def get_by_id(minor_head_id):
        ...
        cursor.execute('''
            SELECT minor_head_id, major_head_id, minor_head_name, 
                   CAST(minor_head_id AS TEXT) as code, '' as description
            FROM minor_heads
            WHERE minor_head_id = ? AND is_active = 1
        ''', (minor_head_id,))
    ...
    def get_by_name_and_major(name, major_head_id):
        ...
        cursor.execute('''
            ...
            FROM minor_heads
            WHERE minor_head_name = ? AND major_head_id = ? AND is_active = 1
        ''', (name, major_head_id))
    ...
    def create(company_id, major_head_id, minor_head_name, opening_balance_cy=0.0,
        ...
        try:
            ...
            cursor.execute(
                'SELECT MAX(display_order) FROM minor_heads WHERE company_id = ? AND major_head_id = ?',
                (company_id, major_head_id)
            )
            max_order = cursor.fetchone()[0]
            display_order = (max_order or 0) + 1
            ...
            minor_head_id = cursor.lastrowid
            conn.commit()
            conn.close()
    ...
    def update(minor_head_id, major_head_id, minor_head_name, opening_balance_cy=None,
        ...
        try:
            if opening_balance_cy is not None and opening_balance_py is not None:
                cursor.execute('''
                    UPDATE minor_heads
                    SET minor_head_name = ?, major_head_id = ?,
                        opening_balance_cy = ?, opening_balance_py = ?
                    WHERE minor_head_id = ?
...
class Grouping:
    ...
    def get_all(company_id=None, minor_head_id=None, major_head_id=None):
        ...
        if company_id and minor_head_id:
            cursor.execute('''
                ...
                FROM groupings
                WHERE company_id = ? AND minor_head_id = ? AND is_active = 1
    ...
    def get_by_id(grouping_id):
        ...
        cursor.execute('''
            SELECT grouping_id, minor_head_id, grouping_name, 
                   CAST(grouping_id AS TEXT) as code, '' as description
            FROM groupings
            WHERE grouping_id = ? AND is_active = 1
        ''', (grouping_id,))
    ...
    def get_by_name(name, minor_head_id=None, major_head_id=None):
        ...
        if minor_head_id:
            cursor.execute('''
                ...
                FROM groupings
                WHERE grouping_name = ? AND minor_head_id = ? AND is_active = 1
            ''', (name, minor_head_id))
        elif major_head_id:
            cursor.execute('''
                SELECT grouping_id, grouping_name, minor_head_id, major_head_id, display_order
                FROM groupings
                WHERE grouping_name = ? AND major_head_id = ? AND is_active = 1
            ''', (name, major_head_id))
        else:
            cursor.execute('''
                SELECT grouping_id, grouping_name, minor_head_id, major_head_id, display_order
                FROM groupings
                WHERE grouping_name = ? AND is_active = 1
            ''', (name,))
    ...
    def create(company_id, minor_head_id, grouping_name, opening_balance_cy=0.0,
        ...
        try:
            ...
            cursor.execute(
                'SELECT major_head_id FROM minor_heads WHERE minor_head_id = ?',
                (minor_head_id,)
            )
            result = cursor.fetchone()
            if not result:
            ...
            cursor.execute(
                'SELECT MAX(display_order) FROM groupings WHERE company_id = ? AND minor_head_id = ?',
                (company_id, minor_head_id)
            )
            max_order = cursor.fetchone()[0]
            display_order = (max_order or 0) + 1
